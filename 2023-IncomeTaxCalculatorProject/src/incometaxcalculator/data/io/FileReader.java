...
public abstract class FileReader {
  ...
  protected abstract int returnTXTReceiptId(String values[]);
  protected abstract int returnXMLReceiptId(String values[]);
  ...
  public int checkForReceipt(BufferedReader inputStream)
    ...
    while (!isEmpty(line = inputStream.readLine())) {
      ...
      {
        int receiptId = returnXMLReceiptId(values);
        return receiptId;
      }
      else if (values[0].equals("Receipt"))
      {
        if (values[1].equals("ID:")) {
          int receiptId = returnTXTReceiptId(values);
          return receiptId;
        }
      }
    }
