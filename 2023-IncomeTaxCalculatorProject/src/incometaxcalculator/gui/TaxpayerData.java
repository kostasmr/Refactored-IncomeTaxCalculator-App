...
public class TaxpayerData extends JFrame {
  ...
  public TaxpayerData(int taxRegistrationNumber, TaxpayerManager taxpayerManager) {
    ...
    //IMPROVEMENT TASK : click on the list to see the receipt
    receiptsList.addMouseListener(new MouseListener()
    {
      @Override
      public void mouseClicked(MouseEvent e) {
        // TODO Auto-generated method stub
        int receiptId =  receiptsList.getSelectedValue();
        Receipt receipt = receipts.get(receiptId);
        ViewReceiptFrame viewReceiptFrame = new ViewReceiptFrame(receipt,receiptsModel,taxpayerManager);
        viewReceiptFrame.setVisible(true);
      }
    ...
    btnAddReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        AddReceiptFrame addReceiptFrame = new AddReceiptFrame(taxRegistrationNumber,taxpayerManager,receiptsModel);
        addReceiptFrame.setVisible(true);
        /*IMPROVEMENT TASK : new frame
    ...
    btnDeleteReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if (op == 0) {
          //IMPROVEMENT TASK
          if(!receiptID.getText().equals("")){
            int receiptIDValue = Integer.parseInt(receiptID.getText());
            try {
              //IMPROVEMENT TASK : check if the receipt exists
              if(receiptsModel.contains(receiptIDValue)) {
                taxpayerManager.removeReceipt(receiptIDValue);
                receiptsModel.removeElement(receiptIDValue);
              }
              else {
                JOptionPane.showMessageDialog(null, "This receipt doesn't exists.Try again.");
              }
            } catch (IOException e1) {
              JOptionPane.showMessageDialog(null,
                  "Problem with opening file ." + receiptIDValue + "_INFO.txt");
